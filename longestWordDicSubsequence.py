...
def findWordSubsequence(st, word):
    i = 0
    for s in st:
        if s == word[i]:
            i += 1
...
def findLargestWord(st, wordList):
    print wordList
    wordList = sorted(wordList, key=lambda w: len(w), reverse=True)
    print wordList
    for word in wordList:
        if findWordSubsequence(st, word):
            return word
    return 'None of the words exsits'
...
if __name__ == '__main__':
    ...
    s = 'abppplee'
    print findLargestWord(s, wordList)
